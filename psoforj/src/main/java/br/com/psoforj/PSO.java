...
public class PSO {
	...
	public PSO(Function function, int numberOfParticles, int dimension, double bottomDomainLimit, double topDomainLimit, TopologyType topologyType) {
	...
	public double[] optimize(double initialInertiaCoefficient, double finalInertiaCoefficient, double cognitiveCoefficient, double socialCoefficient, int numberOfIteractions){
		double[] populationBestFitnessHistory = new double[numberOfIteractions];
	...
	private void updatePopulationValues(double inertiaCoefficient, double finalInertiaCoefficient, double cognitiveCoefficient, double socialCoefficient) {
		double bestPopulationPositionFitness = this.function.fitness(this.population.getBestPosition());
		int populationSize = this.population.getSubPopulationList().size();
	...
	private void updateSubPopulationValues(double inertiaCoefficient, double cognitiveCoefficient, double socialCoefficient, SubPopulation subPopulation) {
		double bestSubPopulationPositionFitness = this.function.fitness(subPopulation.getBestPosition());
	...
	private void updateSubPopulationValues(double cognitiveCoefficient, double socialCoefficient, SubPopulation subPopulation) {
		double bestSubPopulationPositionFitness = this.function.fitness(subPopulation.getBestPosition());
	...
	private void updateParticleValues(double inertiaCoefficient, double cognitiveCoefficient, double socialCoefficient, Particle particle, SubPopulation subPopulation){
	...
	private void updateParticleValues(double cognitiveCoefficient, double socialCoefficient, Particle particle, SubPopulation subPopulation){
		int dimension = subPopulation.getDimension();
		...
		for (int i = 0; i < dimension; i++) {
			...
			double newVelocity = PSOConstants.CLERC_KENNEDY_CONSTRICTION_FACTOR * (particleVelocity + cognitivePart + socialPart);
			double newPosition = particleCurrentPosition + newVelocity;
